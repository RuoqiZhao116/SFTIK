...
class SFTIK(nn.Module):
    ...
    def forward(self, pre_angle,pre_imu, image):
        ...
        for blk in self.former_ts_blocks:
            ts_patches = blk(ts_patches)

        for blk in self.former_img_blocks:
            img1_patches = blk(img1_patches)
