...
class CustomPatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.patch_size = (224, patch_width)  # Patch size (224,16)
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=self.patch_size, stride=(224,patch_width), bias=bias)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

        self.blocks = nn.ModuleList([
                Block(embed_dim, 12, 4., qkv_bias=True, norm_layer=nn.LayerNorm)
                for i in range(6)])
